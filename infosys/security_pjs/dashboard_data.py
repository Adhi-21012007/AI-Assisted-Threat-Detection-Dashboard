...
def ensure_soc_schema(con):
    ...
    # Intelligence tables share the same company database and event lifecycle.
...
def event_detail(path, record_id):
    ...
    try:
        ...
        event=_item(row);event['actions']=[dict(x) for x in con.execute('SELECT * FROM incident_actions WHERE security_event_id=? ORDER BY created_at DESC',(record_id,)).fetchall()];event['incidents']=[dict(x) for x in con.execute('SELECT i.* FROM incidents i JOIN incident_event_links l ON l.incident_id=i.id WHERE l.security_event_id=?',(record_id,)).fetchall()]
...
def record_action(path, record_id, action, note, analyst_id):
    ...
    try:
        ...
        if status:
            con.execute('UPDATE security_events SET status=? WHERE id=?',(status,record_id))
            # Correlated incidents use the same explicit lifecycle as their linked event.
            con.execute('UPDATE incidents SET status=?,updated_at=? WHERE id IN (SELECT incident_id FROM incident_event_links WHERE security_event_id=?)',(status,datetime.now(timezone.utc).isoformat(),record_id))
...
def intelligence_records(path, finding_type=None, limit=50):
    con=_con(path)
    try:
        clauses=[];values=[]
        if finding_type:clauses.append('sf.finding_type=?');values.append(finding_type)
        values.append(min(max(int(limit),1),200))
        query='SELECT sf.*,se.event_id,se.severity,se.risk_score,se.status,se.ai_threat_type,se.ai_recommended_action FROM security_findings sf JOIN security_events se ON se.id=sf.security_event_id'
        if clauses:query+=' WHERE '+' AND '.join(clauses)
        query+=' ORDER BY sf.id DESC LIMIT ?'
        return [dict(x) for x in con.execute(query,values).fetchall()]
    finally:con.close()
def incidents(path, limit=100):
    con=_con(path)
    try:
        return [dict(x) for x in con.execute('SELECT i.*,COUNT(l.security_event_id) event_count FROM incidents i LEFT JOIN incident_event_links l ON l.incident_id=i.id GROUP BY i.id ORDER BY i.updated_at DESC LIMIT ?',(min(max(int(limit),1),200),)).fetchall()]
    finally:con.close()
def add_tool_record(path, tool_type, payload, source='ANALYST'): 
    now=datetime.now(timezone.utc).isoformat();title=(payload.get('title') or '').strip();status=(payload.get('status') or 'OPEN').upper();severity=(payload.get('severity') or 'LOW').upper();risk=payload.get('risk_score');source=(payload.get('source') or source).strip().upper()
    if not title:raise ValueError('title is required')
    if severity not in SEVERITIES:raise ValueError('invalid severity')
    allowed={'VULNERABILITY':{'OPEN','INVESTIGATING','MITIGATING','RESOLVED','ACCEPTED','FALSE POSITIVE'},'PENTEST':{'PLANNED','IN PROGRESS','OPEN','RESOLVED','ACCEPTED','FALSE POSITIVE'}}
    if tool_type in allowed and status not in allowed[tool_type]:raise ValueError('invalid lifecycle status')
    if risk not in (None,'') and (not str(risk).isdigit() or not 0<=int(risk)<=100):raise ValueError('risk score must be 0-100')
    con=_con(path)
    try:
        con.execute('INSERT INTO security_tool_records(tool_type,asset,title,description,severity,risk_score,status,evidence_reference,recommendation,source,metadata_json,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',(tool_type,(payload.get('asset') or '')[:200],title[:300],(payload.get('description') or '')[:4000],severity,int(risk) if risk not in (None,'') else None,status[:40],(payload.get('evidence_reference') or '')[:500],(payload.get('recommendation') or '')[:2000],source,json.dumps({'related_event_id':(payload.get('related_event_id') or '')[:100]}),now,now));con.commit()
    finally:con.close()
